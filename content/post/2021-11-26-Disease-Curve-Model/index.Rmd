...
![](C:\Users\sithj\Documents\Github\academic-website\content\post\2021-11-24-Play-TH-Map-2\DPC_model.png)
...
เราจะใช้ functions `The sim_` จาก package `epifitter` เพื่อ จำลองข้อมูลจากโมเดลแบบต่าง เพื่อมาศึกษา โดยมี 6 ค่าที่เราจะลองปรับ ดู
...
เมื่อ `n` มีค่ามากกว่า 1  When n is greater than one, replicated epidemics (e.g. replicated treatments) are produced and a level of noise (experimental error) should be set in the alpha argument. These two arguments combined set will generate random_y values, which will vary randomly across the defined number of replicates.
